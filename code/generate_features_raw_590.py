...
samples = pd.read_csv(r'D:\Documenten\TUdelft\thesis\mep_veldhuis\data\noc590.txt', index_col=0)  
...
data = {}
for files in glob.glob(r"D:\Documenten\TUdelft\thesis\mep_veldhuis\data\raw\*"):
    file = open(files, "r")
    line1 = file.readline()
    ...
    for lines in file:
        ...
        sample = line[0]
        if sample not in samples.index: # only keep the 590
            continue
        locus = line[1]
...
for sample in data:
    if sample not in Features:
        ...
        Features[sample]["NOC"] = samples.loc[sample]["NOC"] # instead of setting it to 6
        allelecount[sample] = []
...
# MAC_loci = pd.DataFrame(0, index=range(5), columns=lociMarkers)
# for sample in Features:
#     MAC = Features[sample]["MAC"]
#     NOC = int(Features[sample]["NOC"])
#     MAC_locis = []
#     for locus in Features[sample]["Locus"]:
#         if Features[sample]["Locus"][locus]["AlleleCount"+"_"+locus] == MAC and (len(MAC_locis) == 0 or MAC_locis[0] == 'SE33'):
#             MAC_locis.append(locus)
#             MAC_loci[locus][NOC-1]+=1
# print(MAC_loci)
...
with open(r"D:\Documenten\TUdelft\thesis\mep_veldhuis\data\new_features.txt", "w") as outfile:
